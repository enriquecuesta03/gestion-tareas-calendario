...
function MetricsDashboard({ 
    ...
    // 1. INTERCEPTAMOS LOS COLORES PARA FORZAR LA PALETA ELEGANTE
    const datosConColoresPremium = datosGraficoEstado.map(dato => {
        if (dato.name === 'Por Hacer') return { ...dato, color: '#f59e0b' }; // Ámbar
        if (dato.name === 'En Progreso') return { ...dato, color: '#3b82f6' }; // Azul
        if (dato.name === 'Completado') return { ...dato, color: '#10b981' }; // Verde Kora
        return dato;
    });

    // 2. TOOLTIP PERSONALIZADO (Respeta el modo oscuro)
    ...
    // Función para mostrar las etiquetas por fuera del anillo (Solo si hay valor)
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <style>{`
                ...
                /* CUSTOMIZACIÓN DE LA LEYENDA DE RECHARTS */
                        ...
                        <div className="backlog-list">
                            ...
                            {backlogOrdenado.map(tarea => (
                                <div 
                                    ...
                                    onClick={() => { setTareaEnVista(tarea); cargarComentarios(tarea.id); }}
                                >
